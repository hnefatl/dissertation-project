...
runDeoverload
    :: Deoverload a -> LoggerT NameGenerator (Except Text a, DeoverloadState)
...
addClassEnvironment ce =
    modify (\s -> s { classEnvironment = M.union ce (classEnvironment s) })
...
addDictionaries dicts = do
    ...
    unless (M.null intersection)
        $  throwError
        $  "Clashing dictionary types: "
        <> showt intersection
    modify (\s -> s { dictionaries = M.union existingDicts dicts })
addScopedDictionaries
    :: M.Map TypePredicate VariableName -> Deoverload a -> Deoverload a
...
getDictionary p = gets (M.lookup p . dictionaries) >>= \case
    Nothing ->
        throwError $ "Dictionary " <> showt p <> " not found in environment"
    Just v -> return v
...
deoverloadRhs (HsUnGuardedRhs expr) = case expr of
    e@(HsExpTypeSig loc _ t@(HsQualType _ simpleType)) -> do
        ...
        let dictArgs = M.fromList $ zip constraints args
            funArgs =
                [ HsPVar $ HsIdent $ unpack arg | VariableName arg <- args ]
...
-- The wrapping expression is a lambda taking dictionaries around the inner expression and has modified type
            outerExp =
                HsExpTypeSig loc (HsLambda loc funArgs innerExp) outerType
        return $ HsUnGuardedRhs $ if null constraints
            then innerExp
            else outerExp
    _ ->
        throwError
            $ "Found rhs without top-level type annotation: forgot to run type tagger?\n"
            <> showt expr
deoverloadRhs _ = throwError "Unsupported RHS in deoverloader"

deoverloadExp :: HsExp -> Deoverload HsExp
deoverloadExp (HsExpTypeSig l (HsVar n) t@(HsQualType origTagQuals origSimpleType))
    = do
        ks <- getKinds
        ...
        requiredQuals <- tryGetType varName >>= \case
            ...
            Just t'@(Quantified _ (Qualified varQuals varSimpleType)) -> do
                ...
                fmap catMaybes
                    $ forM (S.toList $ applySub sub varQuals)
                    $ \varQual -> do
...
deoverloadExp (HsExpTypeSig l lit@(HsLit _) (HsQualType _ t)) =
    return $ HsExpTypeSig l lit (HsQualType [] t)
deoverloadExp (HsExpTypeSig l (HsApp f e) _) = do
    fExp@(HsExpTypeSig _ _ t) <- deoverloadExp f
    eExp                      <- deoverloadExp e
    case t of
        HsQualType [] (HsTyFun _ retType) ->
            return $ HsExpTypeSig l (HsApp fExp eExp) (HsQualType [] retType)
        _ -> throwError $ "Got non-function type in application: " <> showt t
deoverloadExp HsInfixApp{} = throwError
    "Infix applications should have been replaced by the type inferrer"
deoverloadExp (HsLambda a pats e) = HsLambda a pats <$> deoverloadExp e
deoverloadExp (HsIf c e1 e2) =
    HsIf <$> deoverloadExp c <*> deoverloadExp e1 <*> deoverloadExp e2
...
deoverloadExp (HsExpTypeSig l e (HsQualType _ t)) =
    HsExpTypeSig l <$> deoverloadExp e <*> pure (HsQualType [] t)
deoverloadExp e =
    throwError $ "Unsupported expression in deoverloader: " <> showt e

-- |Convert eg. `(Num a, Monoid b) => a -> b -> ()` into `Num a -> Monoid b -> a -> b -> ()` to represent the dicts.
deoverloadType :: HsQualType -> HsQualType
deoverloadType (HsQualType constraints t) =
    HsQualType [] $ foldr (HsTyFun . deoverloadAsst) t constraints
...
deoverloadQualType (Qualified quals t) =
    makeFun (map deoverloadTypePredicate $ toList quals) t
...
addDictArgs e@(HsExpTypeSig loc _ (HsQualType [] (HsTyFun argType retType))) (arg : args)
    = addDictArgs a args
  where
    r = HsExpTypeSig loc arg (HsQualType [] argType)
    a = HsExpTypeSig loc (HsApp e r) (HsQualType [] retType)
addDictArgs e _ =
    throwError $ "Invalid expression in deoverloader: " <> showt e

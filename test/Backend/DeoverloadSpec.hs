...
test = testGroup "Deoverload"
    [ let a = HsTyVar $ HsIdent "a"
          b = HsTyVar $ HsIdent "b"
          num = UnQual $ HsIdent "Num"
          monoid = UnQual $ HsIdent "Monoid"
          numa = HsTyApp (HsTyCon num) a
          monoidb = HsTyApp (HsTyCon monoid) b
          unit = HsTyCon $ Special HsUnitCon
          -- actual   = (Num a, Monoid b) => a -> b -> b
          input = HsQualType [(num, [a]), (monoid, [b])] $ HsTyFun a (HsTyFun b b)
          actual = deoverloadType input
          -- expected = Num a -> Monoid b -> (Num a -> a) -> (Monoid b -> b) -> b
          expected = HsTyFun numa $ HsTyFun monoidb $ HsTyFun (HsTyFun numa a) $ HsTyFun (HsTyFun monoidb b) b
      in testCase (unpack $ synPrint input) $ assertEqual (unpack $ synPrint actual) expected actual
    , let

...
test :: TestTree
test = let
        [a, b] = [ TypeVariable s KindStar | s <- map Id ["a", "b"] ]
        [ta, tb] = map TypeVar [a, b]
        [num, fractional] = map Id ["Num", "Fractional"]
    in
    ...
    ,
        ...
        in testBindings s
            ...
            , (Id "y", Quantified S.empty $ Qualified q ta) ]
    ,
        let s = "x = (\\y -> False && y)"
        in testBindings s
    ...
    ,
        ...
        in testBindings s
            [ (Id "even", Quantified (S.singleton a) $ Qualified (S.singleton $ IsInstance num ta) (helper ta))
            , (Id "odd", Quantified (S.singleton b) $ Qualified (S.singleton $ IsInstance num tb) (helper tb)) ]
    ,
